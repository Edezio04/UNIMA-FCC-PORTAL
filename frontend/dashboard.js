    ...
    try {
        ...
        if (upcomingContainer) {
            if (eventsList.length === 0) {
                upcomingContainer.innerHTML = `<div class="event-item"><h4>No Upcoming Activities</h4><p>New events will appear here.</p></div>`;
            } else {
                let html = "";
                eventsList.slice(0, 5).forEach((event) => {
                    ...
                    html += `<div class="event-item"><h4>${escapeHtml(title)}</h4><p><i class="fa-regular fa-clock"></i> ${parsedDate.toLocaleString()}</p><p><i class="fa-solid fa-location-dot"></i> ${escapeHtml(location)}</p></div><hr class="card-divider">`;
                });
...
async function loadEvents() {
    ...
    if (!container) return;
    container.innerHTML = `<div class="card"><h3>Loading Events...</h3></div>`;
    try {
        const response = await fetch("/events?t=" + Date.now());
        const rawEvents = await response.json();
        let html = `<div class="card"><div class="card-header-inside"><i class="fa-solid fa-calendar"></i><h2>Upcoming Events</h2></div><button class="btn btn-primary" onclick="renderAddEventForm()">+ Add Event</button><hr class="card-divider">`;
...
function renderAddEventForm() {
    const container = document.getElementById("dashboardContent");
    container.innerHTML = `<div class="card"><h2>Create Event</h2><form onsubmit="handleCreateEvent(event)"><label for="eventTitle">Title</label><input id="eventTitle" placeholder="Event title" required class="w-100"><br><br><label for="eventDescription">Description</label><textarea id="eventDescription" placeholder="Description" class="w-100"></textarea><br><br><label for="eventDate">Date & Time</label><input type="datetime-local" id="eventDate" required class="w-100"><br><br><label for="eventLocation">Location</label><input id="eventLocation" placeholder="Location" required class="w-100"><br><br><button type="submit" class="btn btn-primary">Save Event</button> <button type="button" class="btn btn-secondary" onclick="loadEvents()">Cancel</button></form></div>`;
}
...
async function loadBibleStudies() {
    ...
    try {
        ...
        let html = `<div class="card"><div class="card-header-inside"><i class="fa-solid fa-book-bible"></i><h2>Bible Study Resources</h2></div><button class="btn btn-primary" onclick="renderUploadForm()">+ Upload Document</button><hr class="card-divider">`;
...
function renderUploadForm() {
    const container = document.getElementById("dashboardContent");
    container.innerHTML = `<div class="card"><h2>Upload Bible Study</h2><form onsubmit="handleStudyUpload(event)"><label>Title</label><input id="studyTitle" required class="w-100"><br><br><label>Description</label><textarea id="studyDescription" class="w-100"></textarea><br><br><label>Document</label><input type="file" id="studyDocument" required><br><br><button type="submit" class="btn btn-primary">Upload</button> <button type="button" class="btn btn-secondary" onclick="loadBibleStudies()">Cancel</button></form></div>`;
}
...
async function loadPrayers() {
    ...
    try {
        ...
        let html = `<div class="card"><div class="card-header-inside"><i class="fa-solid fa-hands-praying"></i><h2>Prayer Requests</h2></div><button class="btn btn-primary" onclick="renderAddPrayerForm()">+ Submit Prayer</button><hr class="card-divider">`;
